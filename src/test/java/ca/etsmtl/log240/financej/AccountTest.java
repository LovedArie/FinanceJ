...
private class TestValideAccountWindow extends WindowHandler {
    ...
    public Trigger process(Window window) throws Exception {
        ...
        WindowInterceptor.init(accountsButton.triggerClick()).process(new TestAccountValideWindow(getRandomName(),"Savings")).run();
        ...
        for (String account : accountsNomsAlphaNum) {
            WindowInterceptor.init(accountsButton.triggerClick()).process(new TestAccountValideWindow(account,"Savings")).run();
        }
        ...
        WindowInterceptor.init(accountsButton.triggerClick()).process(new TestAccountValideWindow(accountsNomsAlphaNum.get(random.nextInt(49)),"")).run();
        ...
        for (String desc : descriptionsValides) {
            WindowInterceptor.init(accountsButton.triggerClick()).process(new TestAccountValideWindow(accountsNomsAlphaNum.get(random.nextInt(49)),desc)).run();
        }
...
private class TestAccountValideWindow extends WindowHandler {
    private String name;
    private String description;

    public TestAccountValideWindow(String name, String description) {
        this.name = name;
